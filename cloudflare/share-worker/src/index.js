...
async function signSession(env, role) {
  const payload = b64url(encoder.encode(JSON.stringify({ role, exp: Math.floor(Date.now()/1000)+SESSION_SECONDS, project: "cheatvision" })));
...
async function verifySession(request, env) {
  ...
  try {
    ...
    if (payload.project !== "cheatvision" || payload.exp < Math.floor(Date.now()/1000)) return null;
